...
3. 页面调用 `CreateHealthRecordController.createHealthRecord`
4. `HealthRecordService` 先调用 `/ai/extract`
5. 客户端校验提取结果中的时间字段
6. 将健康事件写入 `health_events`
7. 若有图片，复制到应用私有目录并写入 `attachments`
8. 返回列表页并提示“记录已保存”

### 流程 2：浏览健康记录

1. 用户进入 `/records`
2. 列表页读取本地 `health_events`，按 `eventEndTime` 倒序展示
3. 用户进入 `/records/:id`
4. 详情页分别读取记录与附件
5. 页面展示原始文本、摘要、备注、时间和图片预览
...
- `/ai/extract` 返回非法时间或开始时间晚于结束时间：保存取消，页面提示失败
