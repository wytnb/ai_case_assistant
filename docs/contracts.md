...
当前批次先落地“手动生成周报 / 月报 / 季报”的最小字段集，优先保证可生成、可落库、可回显。  
其中 `week` 按最近 7 天实现，`month` 按最近 30 天实现，`quarter` 按最近 90 天实现。  
自动调度、导出与分享能力不在本批落地。
...
| reportType | String | 是 | 事实字段 | 报告类型 | 当前允许 `week`、`month`、`quarter` |
...
1. 当前批次支持手动生成 `week`、`month`、`quarter` 三种报告。
2. `adviceJson` 存储 `string[]` 的 JSON 字符串，详情页展示前需反序列化。
3. 报告详情主展示内容优先使用 `markdown`，列表摘要使用 `summary`。
4. `week` 对应最近 7 天，`month` 对应最近 30 天，`quarter` 对应最近 90 天。
5. 当范围内没有健康事件时，仍允许提交 `events: []` 生成并保存空报告。
6. 当本地已存在同一 `reportType + rangeStart + rangeEnd` 的报告时，应覆盖更新并清理重复记录，而不是新增重复条目。
...
{
  "reportType": "month",
  "rangeStart": "2026-02-06T00:00:00+08:00",
...
1. 请求体必须显式传 `reportType`、`rangeStart`、`rangeEnd`、`events`。
2. `reportType` 当前允许 `week`、`month`、`quarter`。
3. 响应必须是对象。
4. `title`、`summary`、`advice`、`markdown` 为当前最小必需字段。
5. `advice` 必须是字符串数组（`string[]`）。
6. 页面主展示优先使用 `markdown`，列表摘要使用 `summary`。
...
5. 报告当前支持手动生成 `week`、`month`、`quarter`；时间范围分别按最近 7 天、30 天、90 天计算。
6. 同一 `reportType + rangeStart + rangeEnd` 的报告在本地按覆盖更新处理，不保留重复记录。
7. `HealthEvent.symptomSummary` 保留为纯文本摘要字段，不改成 JSON。
8. `Attachment.filePath` 统一收敛为应用可控目录路径，不混用绝对路径与临时 URI。
9. `FollowupSession`、`ExtractResult` 等 AI 能力相关表延后到对应批次再落地。
