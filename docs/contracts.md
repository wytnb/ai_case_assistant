...
3. 当前已使用时间字段语义如下：
   - `eventTime`：健康事件时间，当前保存记录时写入 `DateTime.now()`
   - `createdAt`：记录创建时间
   - `updatedAt`：记录最后更新时间
...
2. `symptomSummary`、`notes` 允许为空，用于承接 AI 返回不完整时的兜底结果。
...
| eventTime | DateTime | 是 | 记录时间 | 当前保存时写入 `DateTime.now()` |
...
| notes | String? | 否 | AI 提取后的备注 | 优先使用远程返回，否则客户端兜底生成 |
| createdAt | DateTime | 是 | 创建时间 | 保存时写入当前时间 |
...
5. 当前没有 `status`、`inputType`、`followup` 等附加字段。
...
1. 新增记录流程在入库前必须拿到该对象。
2. 当前即使远程返回缺字段，也会通过客户端兜底补出 `symptomSummary` 和 `notes`。
3. 当前没有独立的 ExtractResult 数据表。
...
5. `notes` 若缺失、类型错误或空字符串，客户端会基于 `rawText` 和摘要生成兜底备注。
6. 当前提取请求不会携带图片内容。
...
```json
{
  ...
  "events": [
    {
      "id": "string",
      "eventTime": "2026-03-15T08:30:00.000",
      "sourceType": "text",
      "rawText": "string",
      "symptomSummary": "string",
      "notes": "string"
...
1. 响应不是对象：失败。
2. 字段缺失或为空：生成本地兜底摘要 / 备注。
3. 网络或上游失败：新增记录流程失败，页面提示“保存失败，请稍后重试”。
