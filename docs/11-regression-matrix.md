...
|---|---|---|---|---|---|---|
| 首页与主入口 | 必跑 | 页面或导航变化时必跑 | 发布 / 验收时必跑 | 仅备用 | 首页布局、免责弹窗、追问模式开关、主导航变化 | 日常改动优先模拟器；若 `/records` 为根路由，返回应回首页 |
| 首页三个入口跳转 | 必跑 | 必跑 | 发布 / 验收时必跑 | 仅备用 | 首页入口文案、入口卡片、路由变化 | Web 仅能补基础打开；需覆盖首页 -> 记录列表 / 新增记录的回退链路 |
| 新增记录纯文本主链路 | 必跑 | 必跑 | 发布 / 验收时必跑 | 可作为文本链路备用 | `/records/new` UI、提交、路由、`/ai/intake` 纯文本链路变化 | 不选图片时模拟器足够 |
| 列表页基础打开 | 必跑 | 必跑 | 发布 / 验收时必跑 | 仅备用 | `/records` 页面结构、双 tab、日期筛选、删除入口、导航变化 | 基础打开不要求真机；筛选/删除交互回归时补跑 `integration_test/health_record_list_page_test.dart`；根路由返回应回首页 |
| 记录详情基础打开 | 必跑 | 必跑 | 发布 / 验收时必跑 | 仅备用 | `/records/:id` 字段展示、删除入口、追加补充入口、导航变化 | 若涉及图片预览则升级到真机；根路由返回应回 `/records?tab=records` |
...
| 追问页与会话恢复 | 必跑 | 页面或主链路变化时必跑 | 发布 / 验收时必跑 | 仅备用 | `/intake/:id` 页面、状态恢复、强制结束、草稿删除变化 | 纯文本追问流程模拟器足够；根路由返回应回 `/records?tab=drafts` |
